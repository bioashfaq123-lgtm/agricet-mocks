    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="card p-8 max-w-md w-full text-center">
          ...
          <h2 className="text-2xl font-black text-gray-900 mb-2">AGRICET {year} Paper</h2>
      ...
      <div className="bg-white border-b sticky top-0 z-40 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium">📋 AGRICET {year} Official Paper</span>
          </div>
        ...
        <div className="card p-6">
          ...
          <div className="space-y-3">
            {current?.options.map((opt, idx) => {
              ...
              return (
                <button key={idx} onClick={() => answered === undefined && setAnswers(p => ({ ...p, [current.id]: idx }))}
                  ...
                  <span>{opt}</span>
                  {answered !== undefined && idx === current.correct && <CheckCircle className="w-5 h-5 text-green-600 ml-auto" />}
                  {answered !== undefined && idx === answered && idx !== current.correct && <X className="w-5 h-5 text-red-500 ml-auto" />}
                </button>
              );
            })}
          </div>
